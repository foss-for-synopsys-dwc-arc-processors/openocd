...
#else
	pc = buf_get_u32(arc32->core_cache->reg_list[PC].value, 0, 32);
#endif
...
#else
		buf_set_u32(arc32->core_cache->reg_list[PC].value, 0, 32, address);
		arc32->core_cache->reg_list[PC].dirty = 1;
		arc32->core_cache->reg_list[PC].valid = 1;
#endif
...
#else
		resume_pc = buf_get_u32(arc32->core_cache->reg_list[PC].value,
#endif
			0, 32);
...
#else
		arc32->core_cache->reg_list[PC].dirty,
		arc32->core_cache->reg_list[PC].valid);
#endif
...
# if 0
	if ((arc32->core_cache->reg_list[PC_REG].valid == 1) &&
		(resume_pc == buf_get_u32(arc32->core_cache->reg_list[PC_REG].value,
			0, 32))) {
#else
	if ((arc32->core_cache->reg_list[PC].valid == 1) &&
		(resume_pc == buf_get_u32(arc32->core_cache->reg_list[PC].value,
			0, 32))) {
#endif
...
#else
		value = buf_get_u32(arc32->core_cache->reg_list[PC].value, 0, 32);
#endif
